...
import {
  addToCart,
  cartWillFull,
} from '../../pages/fileCentricCart/store/cart';
...
const GridView = ({
  ...
  function exportFiles() {
    ...
    const newFileIDS =
      fileIDs !== null
        ? selectedIDs.filter(e => !fileIDs.find(a => e === a)).length
        : selectedIDs.length;
  ...
  */
  function onRowsSelect(curr, allRowsSelected, rowsSelected, displayData) {
    rowSelectionEvent(
      displayData.map(d => d.data[primaryKeyIndex]),
      rowsSelected
    );
    setSelectedIDs([...new Set(customOnRowsSelect(data, allRowsSelected))]);
    ...
    if (selectedFileNames) {
      setSelectedRowFileNames([
        ...new Set(selectedFileNames(data, allRowsSelected)),
      ]);
    }
  }
  ...
  const tooltipComponent = (
    <>
      ...
      {messageStatus ? (
        <div
          className={classes.messageBottom}
          style={tooltipStyle(tooltipMessage)}
        >
          {' '}
