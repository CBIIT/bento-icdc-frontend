  ...
  return (
    ...
    >
      ...
      <Grid item xs={11} className={classes.propertyContainer}>
        ...
        <Grid item xs={12}>
          <span className={classes.title}>Study:</span>
          <Link to={`/study/${studyDesignation}`}>
            <span className={classes.contentLink}>{studyDesignation}</span>
          </Link>
        </Grid>
        <Grid item xs={12}>
          <span className={classes.title}>Case:</span>
          <Link to={`/case/${caseID}`}>
            <span className={classes.contentLink}>{caseID}</span>
          </Link>
        </Grid>
        <Grid item xs={12}>
          <span className={classes.title}>Sample Anatomic Site:</span>
          <span className={classes.content}>{sampleSite}</span>
        </Grid>
        <Grid item xs={12}>
          <span className={classes.title}>Sample Type:</span>
          <span className={classes.content}>{sampleType}</span>
        </Grid>
        <Grid item xs={12}>
          <span className={classes.title}>Sample Pathology:</span>
          <span className={classes.content}>{pathology}</span>
        </Grid>
      </Grid>
    </Grid>
  );
