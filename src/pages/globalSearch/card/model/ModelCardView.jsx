    ...
    >
      ...
      <Grid item xs={11} className={classes.propertyContainer}>
        <div>
          ...
          <span className={classes.cardTitle}>{highlightKey()}</span>
        </div>
        <Grid item xs={12}>
          <span className={classes.title}>Data Model Node:</span>
          <span className={classes.content}>{nodeName}</span>
        </Grid>
        {propertyName && (
          <Grid item xs={12}>
            <span className={classes.title}>Property Name:</span>
            <span className={classes.content}>{propertyName}</span>
          </Grid>
        )}
        {description && (
          <Grid item xs={12}>
            <span className={classes.title}>Property Description:</span>
            <span className={classes.content}>{description}</span>
          </Grid>
        )}
        {required && (
          <Grid item xs={12}>
            <span className={classes.title}>Property Required:</span>
            <span className={classes.content}>{required}</span>
          </Grid>
        )}
        {propertyType && (
          <Grid item xs={12}>
            <span className={classes.title}>Property Type:</span>
            <span className={classes.content}>{propertyType}</span>
          </Grid>
        )}
        <Grid item xs={12}>
          <span className={classes.title}>Page Link:</span>
          <Link to="/icdc-data-model">
            <span className={classes.contentLink}>
              Data Model Navigator page
            </span>
          </Link>
        </Grid>
      </Grid>
