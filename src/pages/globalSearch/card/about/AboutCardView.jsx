...
const AboutCardView = ({ searchText = '', data, classes, index = 0 }) => {
  // const results = data?.text?.split().map((result) => result.replaceAll('$', ''));
  const results = data?.text.split('$');
  function getHighlightedText(text = [], highlight = '') {
    // Split on highlight term and include term into parts, ignore case
    const textString = text.reduce(
      (searchResults, currentString, currentIndex) => {
    ...
    >
      ...
      <Grid item xs={11} className={classes.propertyContainer}>
        ...
        <div className={classes.text}>
          {getHighlightedText(results, searchText)}
        </div>
