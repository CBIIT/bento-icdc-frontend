...
async function getAllQueryField(searchText, calcOffset, pageSize) {
  ...
  const custodianConfigForTabData = [
    { countField: 'case_count', nameField: 'cases' },
...
const GlobalSearchView = ({ classes, searchparam = '', study2Program }) => {
  ...
  const getSearchSuggestions = async (_config, value, reason) => {
    ...
    const mapOption = SEARCH_PAGE_KEYS.private.map((key, index) =>
      res[key].map(id => id[SEARCH_PAGE_DATAFIELDS.private[index]])
    );
    const option =
      mapOption.length > 0
        ...
        <div className={classes.heroArea}>
          <div>
            <SearchBar
              value={searchText}
              clearable={!false}
              style={{ width: 750, color: 'red !important' }}
            />
          </div>
        </div>
