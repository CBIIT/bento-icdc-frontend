...
function updateColumns(columns, columnList) {
  ...
  const disableViewColumnsList = columnList.filter(
    column => column.viewColumns !== undefined && !column.viewColumns
  );
  disableViewColumnsList.forEach(disabledColumn => {
    const index = columns.findIndex(
      column =>
        column.label.toLowerCase() === disabledColumn.header.toLowerCase()
    );
    if (index !== -1) {
