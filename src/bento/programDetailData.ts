...
interface Column {
  ...
  display: boolean;
  tooltipText?: string;
  role?: string; // Replace `string` with the specific type for `cellTypes.DISPLAY` if available.
  cellType?: string; // Replace `string` with the specific type for `cellTypes.CUSTOM_ELEM` if available.
  linkAttr?: LinkAttributes;
  columnDefaultValues?: Record<number, string>;
  headerType?: string; // Replace `string` with the specific type for `headerTypes.CUSTOM_ELEM` if available.
  icon?: string;
  link?: string;
}
...
export const tableLayOut: TableLayoutItem[] = [
  {
